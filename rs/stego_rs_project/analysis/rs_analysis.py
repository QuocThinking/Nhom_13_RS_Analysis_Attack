...
def analyseLSBs(imageBox, mask, neg_mask):
            ...
            if (row + 1) % maskRow == 0:
                if (column + 1) % maskCol == 0:
                    ...
                    discrimination_values.append({
                        'position': pos,
                        'original': discr_breakimagebox,
                        'mask_applied': discr_mask_breakimagebox,
                        'neg_mask_applied': discr_neg_mask_breakimagebox,
                        'flipped': discr_flip_box,
                        'mask_flipped': discr_mask_flip_box,
                        'neg_mask_flipped': discr_neg_mask_flip_box
                    })
    ...
    if num == 0:
        return 0, discrimination_values, group_positions
    ...
    return message_length, discrimination_values, group_positions

def create_essential_visualizations(img, discrimination_values, group_positions, encoded_percent, mask_size):
    """Tạo 4 biểu đồ như trong hình"""
    
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(12, 8))

    # 1. Original vs Mask Applied
    if discrimination_values:
        original_vals = [d['original'] for d in discrimination_values]
        mask_vals = [d['mask_applied'] for d in discrimination_values]

        ax1.scatter(original_vals, mask_vals, alpha=0.6, s=1, color='blue')
        min_val = min(min(original_vals), min(mask_vals))
        max_val = max(max(original_vals), max(mask_vals))
        ax1.plot([min_val, max_val], [min_val, max_val], 'r--', alpha=0.8)
        ax1.set_xlabel('Original Discrimination')
        ax1.set_ylabel('Mask Applied Discrimination')
        ax1.set_title('Original vs Mask Applied')
        ax1.grid(True, alpha=0.3)

    # 2. Discrimination Difference Distribution
    if discrimination_values:
        differences = np.array(original_vals) - np.array(mask_vals)
        ax2.hist(differences, bins=30, alpha=0.7, color='steelblue', edgecolor='black')
        ax2.axvline(np.mean(differences), color='red', linestyle='--',
                    label=f'Mean: {np.mean(differences):.2f}')
        ax2.set_xlabel('Discrimination Difference (Original - Mask)')
        ax2.set_ylabel('Frequency')
        ax2.set_title('Discrimination Difference Distribution')
        ax2.legend()
        ax2.grid(True, alpha=0.3)

    # 3. R-S Classification Pie Chart
    if discrimination_values:
        r_count = sum(1 for d in discrimination_values if d['original'] < d['mask_applied'])
        s_count = sum(1 for d in discrimination_values if d['original'] > d['mask_applied'])
        total = len(discrimination_values)
        equal_count = total - r_count - s_count

        categories = ['Regular (R)', 'Singular (S)', 'Equal']
        counts = [r_count, s_count, equal_count]
        colors = ['lightblue', 'lightcoral', 'lightgreen']

        if total > 0:
            wedges, texts, autotexts = ax3.pie(counts, labels=categories, colors=colors,
                                             autopct='%1.1f%%', startangle=90)
            ax3.set_title('R-S Group Classification')
    ...
    estimated_data_bits = encoded_percent * total_pixels * 3 if encoded_percent > 0 else 0
    estimated_data_kb = estimated_data_bits / 8 / 1024

    if encoded_percent == 0:
        classification = "INDETERMINATE"
        color = 'gray'
    elif encoded_percent < 0.01:
        classification = "LIKELY CLEAN"
        color = 'green'
    elif encoded_percent < 0.1:
        classification = "POSSIBLY EMBEDDED"
        color = 'orange'
    else:
        classification = "LIKELY EMBEDDED"
        color = 'red'

    summary_text = f"""RS ANALYSIS RESULTS
...
- Probability: {encoded_percent:.6f}
- Percentage: {encoded_percent*100:.4f}%

Estimated Hidden Data:
- Bits: {estimated_data_bits:.0f}
- Kilobytes: {estimated_data_kb:.2f} KB
...
- Status: {classification}"""
    ...
    ax4.text(0.5, 0.01, classification, fontsize=14, weight='bold',
             color=color, verticalalignment='center', horizontalalignment='center')

    plt.tight_layout()
    return fig

def rs_analysis(img_path, output_path=None):
    """RS Analysis - Sử dụng logic gốc hoàn toàn"""
    ...
    gpercent, discrimination_values, group_positions = analyseLSBs(pix, chosen_mask, neg_mask)
    ...
    if gpercent == 0:
        ...
        # Tính R, S cho kết quả
        if discrimination_values:
            R = sum(1 for d in discrimination_values if d['original'] < d['mask_applied'])
            S = sum(1 for d in discrimination_values if d['original'] > d['mask_applied'])
            total_groups = len(discrimination_values)
            R_ratio = R / total_groups if total_groups > 0 else 0
            S_ratio = S / total_groups if total_groups > 0 else 0
        else:
            R = S = 0
            R_ratio = S_ratio = 0

        # Kết luận
        if encodedpercent < 0.01:
            conclusion = "Ảnh KHÔNG có dấu hiệu giấu tin"
        elif encodedpercent < 0.1:
            conclusion = "Ảnh CÓ THỂ có giấu tin"
        else:
            conclusion = "Ảnh CÓ khả năng cao giấu tin"

    # Tạo visualization
    if discrimination_values and output_path:
        print("Generating essential visualizations...")
        mask_size_str = f"{len(chosen_mask[0])} × {len(chosen_mask)}"
        fig = create_essential_visualizations(img, discrimination_values, group_positions, 
                                            encodedpercent, mask_size_str)
    ...
    return {
        ...
        "plot": output_path if output_path else "displayed",
        "encoded_percent": encodedpercent
    }